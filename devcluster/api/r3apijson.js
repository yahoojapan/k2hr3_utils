...
class R3apiJson {
	constructor() {
		...
		this.data = {
			...
			k2hdkc: {													// Slave configuration to K2HDKC cluster
				config:		'/etc/antpickax/chmpx.ini',						// Configuration file path for k2hdkc(chmpx) slave
	...
	configureKeystonTypeOpts(r3apikey, val) {
		// if the value of r3apikey is same with the value of keystone type, 
		// use the r3apikey as the key of the value
		if (r3apikey.localeCompare(this.data.keystone.type) == 0) {
			var tmp = {[r3apikey] : val};
	...
	configure(key, val) {
		...
		this.configureKeystonTypeOpts(r3apikey, val);
